...
// Note: RN doesnt have workers, BUT under the debugger predicate below becames true and it fails badly.
// Hardcode to false (alexnik)
//var runningInWorkerInstance = typeof importScripts === 'function' && self instanceof WorkerGlobalScope;
var runningInWorkerInstance = false;
...
var msrcryptoUtilities = (function () {
    ...
    function toArray(typedArray) {
        ...
        if (getObjectType(typedArray) === "ArrayBuffer") {
            ...
            // If it is a typed array larger than Uint8Array
...
var msrcryptoJwk = (function () {
    ...
    var algorithmMap = {
        ...
        "rsa-oaep": function (algorithm) {
            return "RS-OAEP-" + hashSize(algorithm);
        },
...
function msrcryptoMath() {
    ...
    function MontgomeryMultiplier(modulus) {
        ...
        for (var j = 0; j < 100; j += 1) zeros[j] = 0;
        ...
        function montgomeryMultiply(multiplicant, multiplier, result, ctx) {
            ...
            var m = ctx.m,
                ...
                mPrime = ctx.mPrime,
                m0 = ctx.m0,
                rightI, r0, q, i = 0, j, jm1, t1, t2, carry, rounds = 0;

            // create the temp array
            var temp = zeros.slice(0, s + 1);
            ...
            while (i < s) {
                ...
                for (; i < rounds;) {
                    ...
                    r0 = temp[0] + multiplicant[0] * rightI;
                    ...
                    for (j = 1, jm1 = 0; j < s; jm1 = j, j += 1) {
                        temp[jm1] = temp[j] + m[j] * q + multiplicant[j] * rightI;
                    }
                    temp[jm1] = temp[j];
                    temp[j] = 0;
            ...
            for (i = 0; i < s; i += 1) result[i] = temp[i];
            result.length = s;

            // Subtract modulus
            var needSubtract = cryptoMath.compareDigits(temp, m) > 0;
            cryptoMath.subtract(result, m, needSubtract ? result : ctx.temp2);
        ...
        function modExp(base, exponent, result) {
            ...
            var i, j,
                recode = fixedWindowRecode2(exponent, windowBits).reverse(),
            ...
            for (i = 1; i < baseTableLen; i++) {
                bt[i] = [];
                modMul(bt[i - 1], base, this.m, bt[i], temp1, temp2);
                normalizeDigitArray(bt[i], this.s, true);
            }

            for (i = 0; i < recode.length; i++) {
                for (j = 0; j < windowBits; j++) {
                    this.montgomeryMultiply(partialResult, partialResult, partialResult);
                }
                this.montgomeryMultiply(partialResult, bt[recode[i]], partialResult);
            }
...
if (typeof operations !== "undefined") {
    ...
    msrcryptoGcm.encrypt = function ( p) {
        ...
        //OperationError : If the iv member of normalizedAlgorithm has a length greater than 2^64 - 1 bytes (0xFFFFFFFFF FFFFFFFFF)
        //OperationError : If the additionalData member of normalizedAlgorithm has a length greater than 2^64 - 1 bytes (0xFFFFFFFFF FFFFFFFFF)
        //OperationError : If the tagLength member of normalizedAlgorithm is one of 32, 64, 96, 104, 112, 120 or 128 (default if not provided)
...
function MsrcryptoEntropy() {
    ...
    function collectEntropy() {
        ...
        var prngCrypto = global.crypto || global.msCrypto;       // WARNING: !!! Do not put this in a function (remember polyfill) !!!
...
var prime = (function () {
    ...
    function setupIncrementalTrialDivision(candidate) {
        ...
        if (smallPrimes.length === 0) smallPrimes = primeSeive(MAX_SMALL_PRIMES);
...
var msrcryptoRsaBase = function (keyStruct) {
    ...
    function dToCrt(keyStruct) {

        var d = cryptoMath.bytesToDigits(keyStruct.d),
            e = cryptoMath.bytesToDigits(keyStruct.e),
            n = cryptoMath.bytesToDigits(keyStruct.n),
            temp = new Array(d.length + e.length),
            temp2 = new Array(n.length),
            y = new Array(n.length),
            x = new Array(n.length),
            k = new Array(d.length + e.length),
            nMinusOne = new Array(n.length),
            r,
            t = 0,
            two = cryptoMath.intToDigits(2),
            success = false;

        cryptoMath.subtract(n, cryptoMath.One, nMinusOne);

        // k = de – 1
        cryptoMath.multiply(d, e, temp);
        cryptoMath.subtract(temp, cryptoMath.One, k);

        r = k.slice();

        do {
            cryptoMath.divRem(r, two, r, temp);
            t += 1;
        } while (cryptoMath.isEven(r));
        ...
        for (var i = 0; i <= 100; i++) {
            ...
            var w = msrcryptoPseudoRandom.getBytes(keyStruct.n.length);
            var g = cryptoMath.bytesToDigits(w);

            cryptoMath.reduce(g, nMinusOne, g, temp, temp2);

            // Let y = g^r mod n
            cryptoMath.modExp(g, r, n, y);

            //  If y = 1 or y = n – 1, then go to Step g.
            if (cryptoMath.compareDigits(y, cryptoMath.One) === 0) {
                continue;
            }

            if (cryptoMath.compareDigits(y, nMinusOne) === 0) {
                continue;
            }
            ...
            for (var j = 1; j < t; j++) {
                ...
                // Let x = y^2 mod n
                cryptoMath.modMul(y, y, n, x, temp, temp2);

                // If x = 1, go to Step 5.
                if (cryptoMath.compareDigits(x, cryptoMath.One) === 0) {
                    success = true;
                    break;
                }

                // If x = n – 1, go to Step g.
                if (cryptoMath.compareDigits(x, nMinusOne) === 0) {
                    skip = true;
                    break;
                }
        ...
        if (success) {
            var p = new Array(n.length),
                q = new Array(n.length),
                dp = new Array(n.length),
                dq = new Array(n.length),
                qi = new Array(n.length);

            cryptoMath.subtract(y, cryptoMath.One, p);
            cryptoMath.gcd(n, p, p);

            cryptoMath.divRem(n, p, q, temp);

            if (cryptoMath.compareDigits(p, q) < 0) {
                t = p; p = q; q = t;
            }

            keyStruct.p = cryptoMath.digitsToBytes(p);
            keyStruct.q = cryptoMath.digitsToBytes(q);

            cryptoMath.modInv(q, p, qi);

            cryptoMath.subtract(p, cryptoMath.One, p);
            cryptoMath.reduce(d, p, dp);

            cryptoMath.subtract(q, cryptoMath.One, q);
            cryptoMath.reduce(d, q, dq);

            keyStruct.dp = cryptoMath.digitsToBytes(dp);
            keyStruct.dq = cryptoMath.digitsToBytes(dq);
            keyStruct.qi = cryptoMath.digitsToBytes(qi);

            var ks = JSON.stringify(keyStruct);

            return;
        }

        throw new Error("Prime factors not found");
    }

    function computeD(keyStruct) {

        var e = cryptoMath.bytesToDigits(keyStruct.e),
            p = cryptoMath.bytesToDigits(keyStruct.p),
            q = cryptoMath.bytesToDigits(keyStruct.q),
            pm1 = new Array(p.length),
            qm1 = new Array(q.length),
            mpq = new Array(p.length + q.length);

        // p-1
        cryptoMath.subtract(p, cryptoMath.One, pm1);

        // q-1
        cryptoMath.subtract(q, cryptoMath.One, qm1);

        // (p-1) x (q-1)
        cryptoMath.multiply(pm1, qm1, mpq);

        // e^-1 mod ((p-1) x (q-1))
        return cryptoMath.digitsToBytes(cryptoMath.modInv(e, mpq));
    }
...
if (typeof operations !== "undefined") {
    ...
    msrcryptoRsa.genRsaKeyFromRandom = function (bits) {
        ...
        return {
            privateKey: { n: d2b(n), e: d2b(e), d: d2b(d), p: d2b(p), q: d2b(q), dp: d2b(dp), dq: d2b(dq), qi: d2b(qi) },
            publicKey: { n: d2b(n), e: d2b(e) }
        };
    };

    msrcryptoRsa.generateKeyPair = function (p) {

        if (typeof p.algorithm.modulusLength === 'undefined') throw new Error("missing modulusLength");
        ...
        switch (p.algorithm.modulusLength) {
            ...
            case 2048:
            case 4096:
                keyPair = msrcryptoRsa.genRsaKeyFromRandom(p.algorithm.modulusLength);
                break;
            default:
...
function syncWorker() {
    ...
    function postMessage(data) {
        ...
        try {
            ...
            // Note: result may not always be available e.g. decrypt failed due to a wrong key
            // In this case, explicitly throw an error so that the onerror handler runs - this
            // is needed so that the decrypt promise is correctly rejected (Skype dblewis)
            if (result === null) {
...
/*global Promise,utils,keys,workerManager,typedArraySupport,Uint8Array*/
...
/*global operations,keys,utils,workerManager,keyOperation,cryptoOperation,Promise,
         streamObject*/
...
var publicMethods = {
    ...
    bytesToHexString: function(bytes, separate) {
        /// <signature>
        ///     <summary>Converts an Array of bytes values (0-255) to a Hex string</summary>
        ///     <param name="bytes" type="Array"/>
        ///     <param name="separate" type="Boolean" optional="true">Inserts a separator for display purposes (default = false)</param>
        ///     <returns type="String" />
        /// </signature>
        return msrcryptoUtilities.bytesToHexString(bytes, separate);
    },

    base64ToBytes: function(encodedString) {
        /// <signature>
        ///     <summary>Converts a Base64/Base64Url string to an Array</summary>
        ///     <param name="encodedString" type="String">A Base64/Base64Url encoded string</param>
        ///     <returns type="Array" />
        /// </signature>
        return msrcryptoUtilities.fromBase64(encodedString);
    },
    ...
    hexToBytesArray: function(hexString) {
        ...
        ///     <returns type="Array" />
        /// </signature>
        return msrcryptoUtilities.hexToBytesArray(hexString);
    },

    sha256: msrcryptoSha256.sha256,

    encryptStream: function(algorithm, key) {
        /// <signature>
        /// <summary>Start encryption of a stream</summary>
        ///     <param name="algorithm" type="Algorithm"></param>
        ///     <param name="key" type="Key"></param>
        ///     <returns type="StreamObject" />
        /// </signature>
        var algo = Object.assign({}, algorithm);
        algo.stream = true;
        return msrcryptoSubtle.publicMethods.encrypt(algo, key);
    },

    signStream: function(algorithm, key) {
        /// <signature>
        /// <summary>Start signing a stream</summary>
        ///     <param name="algorithm" type="Algorithm"></param>
        ///     <param name="key" type="Key"></param>
        ///     <returns type="StreamObject" />
        /// </signature>
        var algo = Object.assign({}, algorithm);
        algo.stream = true;
        return msrcryptoSubtle.publicMethods.sign(algo, key);
    },

    digestStream: function(algorithm) {
        /// <signature>
        /// <summary>Start digest of a stream</summary>
        ///     <param name="algorithm" type="Algorithm"></param>
        ///     <returns type="StreamObject" />
        /// </signature>
        var algo = Object.assign({}, algorithm);
        algo.stream = true;
        return msrcryptoSubtle.publicMethods.digest(algo);
    },
