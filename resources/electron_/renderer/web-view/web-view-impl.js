...
const setupMethods = (WebViewElement) => {
  ...
  const createBlockHandler = function (method) {
    return function (...args) {
      const [error, result] = ipcRenderer.sendSync('ELECTRON_GUEST_VIEW_MANAGER_SYNC_CALL', this.getWebContentsId(), method, args)
      if (error) {
  ...
  const createNonBlockHandler = function (method) {
    return function (...args) {
      ...
      ipcRenderer.send('ELECTRON_GUEST_VIEW_MANAGER_ASYNC_CALL', requestId, this.getWebContentsId(), method, args, callback != null)
    }
  }
