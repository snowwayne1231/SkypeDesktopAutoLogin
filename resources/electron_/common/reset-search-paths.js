...
const MICROSOFT_MODULE_PREFIX = '@microsoft/lib/'
...
Module._resolveFilename = function (request, parent, isMain) {
  ...
  } else if (request.startsWith(MICROSOFT_MODULE_PREFIX) && request.length > MICROSOFT_MODULE_PREFIX.length) {
    const slicedRequest = request.slice(MICROSOFT_MODULE_PREFIX.length)
    return path.resolve(BASE_INTERNAL_PATH, `${slicedRequest}${slicedRequest.endsWith('.js') ? '' : '.js'}`)
  } else {
